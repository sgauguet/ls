...
int print_hidden_files(t_file *content) {
	...
	if (!content || !content->filename || content->filename[0] != '.')
		return (0);
	return (1);
}
...
int print_details(t_file *content) 
{
	...
	ft_printf("%C%s %d %s %4d %s %s\n", content->type, access, content->stats.st_nlink, 
		group->gr_name, content->stats.st_size, time, content->filename);
	ft_printf("\033[0m");
	return (0);
}
...
int	print_content(t_arg *args, t_file **content)
{
	...
	if (content == NULL)
		return 1;
	if (*content && ((args->options & op_R) || (args->files && args->directories) || (args->directories && args->directories->next)))
		ft_printf("%s:\n", (*content)->filepath);
	while (*content)
	{
		if (args->options & op_l && (!print_hidden_files(*content) || (args->options & op_a)))
